...
import {
  CustomerDataProd,
  deleteCustomerDataProd,
  addCustomerProd,
  updateCustomerDataProd,
} from './CustomerAPI'
...
const CustomerManagementProd = () => {
  ...
  const clinicList = [...new Set(customerData.map((c) => c.clinicName).filter(Boolean))]
  const cityList = [...new Set(customerData.map((c) => c.city).filter(Boolean))]
  const pincodeList = [...new Set(customerData.map((c) => c.clinicCityArea).filter(Boolean))]
  ...
  const [clinicPincodeFilter, setClinicPincodeFilter] = useState('')
  const [addressPincodeFilter, setAddressPincodeFilter] = useState('')
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await CustomerDataProd()
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      data = data.filter((c) => {
        const addressPincode = getPincodeFromAddress(c.address)
        ...
        return (
          ...
          c?.clinicCityArea?.includes(q) ||
          addressPincode?.includes(q) // ✅ NEW
        )
      })
    }

    if (clinicFilter) {
      data = data.filter((c) => c.clinicName === clinicFilter)
    }
    ...
    if (clinicPincodeFilter) {
      data = data.filter((c) => c.clinicCityArea === clinicPincodeFilter)
    }

    if (addressPincodeFilter) {
      data = data.filter((c) => getPincodeFromAddress(c.address) === addressPincodeFilter)
    }
  ...
  // ✅ THEN lists
  const clinicPincodeList = [...new Set(customerData.map((c) => c.clinicCityArea).filter(Boolean))]
  const addressPincodeList = [
    ...new Set(customerData.map((c) => getPincodeFromAddress(c.address)).filter(Boolean)),
  ]
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await updateCustomerDataProd(updatedFormData.mobile, updatedFormData)
        toast.success('Customer updated successfully')
      } else {
        await addCustomerProd(updatedFormData)
    ...
    if (searchQuery.trim()) {
      ...
      setFilteredData(
        customerData.filter((customer) => {
          ...
          const serviceTypeMatch = (customer?.serviceType || []).some((type) =>
            type.toLowerCase().startsWith(trimmedQuery),
          )
  ...
  const adjustCurrentPageAfterDelete = (updatedLength) => {
    ...
    setCurrentPage((prevPage) => (prevPage > newTotalPages ? Math.max(newTotalPages, 1) : prevPage))
  }
  ...
  const confirmDeleteCustomer = async () => {
    try {
      setDelLoading(true)
      if (isMultiDelete) {
        await Promise.all(selectedMobiles.map((mobile) => deleteCustomerDataProd(mobile)))
        ...
        await deleteCustomerDataProd(customerIdToDelete)
      ...
      {!isAdding ? (
        <>
          <CCardBody style={{ padding: '20px 24px' }}>
            ...
            >
              <h5 className="mb-0 fw-bold theme-heading">Customer Data Management</h5>
            ...
            <CRow className="mb-3">
              ...
              <CCol md={3}>
                <CFormSelect
                  value={clinicPincodeFilter}
                  onChange={(e) => setClinicPincodeFilter(e.target.value)}
                  style={{
                    border: '1px solid var(--color-black)',
                    padding: '10px 12px',
                  }}
                >
                  <option value="">Clinic Pincode</option>

                  {clinicPincodeList.map((p, i) => (
              ...
              <CCol md={3}>
                <CFormSelect
                  value={addressPincodeFilter}
                  onChange={(e) => setAddressPincodeFilter(e.target.value)}
                  style={{
                    border: '1px solid var(--color-black)',
                    padding: '10px 12px',
                  }}
                >
                  <option value="">Address Pincode</option>

                  {addressPincodeList.map((p, i) => (
                    <option key={i} value={p}>
                      {p}
                    </option>
                  ))}
              ...
              <CCol
                md={12}
                className="d-flex justify-content-end align-items-end mt-4  text-end w-100 "
                style={{ color: 'var(--color-bg-black)' }}
              >
                  ...
                  <CTableHead className="pink-table">
                    <CTableRow className="text-center">
                      {[
                        ...
                        <CTableHeaderCell key={title} style={{ padding: '14px 16px' }}>
                          {title}
                        </CTableHeaderCell>
                      ))}
                    </CTableRow>
...
export default React.memo(CustomerManagementProd)
